...
from bot.config import WEB_SEARCH_RESULTS, WEB_PAGE_CHARS
...
def is_schedule_question(text: str) -> bool:
    ...
    return "распис" in lowered and any(
        w in lowered for w in ("завтра", "сегодня", "пар", "занят", "урок")
    )
...
async def fetch_page_text(url: str) -> str:
    ...
    try:
        ...
        logger.error("Page fetch error %s: %s", url, e)
        return ""
...
async def build_web_context(query: str) -> str:
    ...
    if direct_urls:
        chunks = []
        for i, url in enumerate(direct_urls[:WEB_SEARCH_RESULTS], 1):
            page_text = await fetch_page_text(url)
            chunks.append(f"[{i}] Direct link\nURL: {url}\nPage text: {page_text}")
        return "\n\n".join(chunks)
